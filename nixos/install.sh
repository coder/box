...
list_disks() {
  # Whole-block-devices, non-removable, non-loop, non-rom.
  lsblk -d -p -n -b -o NAME,SIZE,MODEL,RM,TYPE \
    | awk '$5=="disk" && $4=="0" { size_h=$2; cmd="numfmt --to=iec --suffix=B "$2; cmd|getline size_h; close(cmd); model=""; for(i=3;i<NF-1;i++) model=model" "$i; print $1"\t"size_h"\t"model }'
}
...
#
# Generated by nixos/install.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ).
